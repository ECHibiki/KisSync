...
let ReferenceCounter = class ReferenceCounter {
    constructor(channel) {
    ...
    unref(caller) {
        if (caller) {
            if (this.references.hasOwnProperty(caller)) {
                ...
                LOGGER.error("ReferenceCounter::unref() called by caller [" + caller + "] but this caller had no active references! " + `(channel: ${this.channelName})`);
                return;
            }
        }
    ...
    checkRefCount() {
        if (this.refCount === 0) {
            if (Object.keys(this.references).length > 0) {
                LOGGER.error("ReferenceCounter::refCount reached 0 but still had " + "active references: " + JSON.stringify(Object.keys(this.references)) + ` (channel: ${this.channelName})`);
                ...
                LOGGER.error("ReferenceCounter::refCount reached 0 but still had " + this.channel.users.length + " active users" + ` (channel: ${this.channelName})`);
...
function Channel(name) {
    ...
    this.modules = {};
    this.logger = new _logger2.default.Logger(path.join(__dirname, "..", "..", "chanlogs", this.uniqueName + ".log"));
    this.users = [];
    this.refCounter = new ReferenceCounter(this);
    this.flags = 0;
    this.id = 0;
    this.ownerName = null;
    this.broadcastUsercount = (0, _throttle.throttle)(() => {
...
Channel.prototype = Object.create(_events.EventEmitter.prototype);
...
Channel.prototype.loadState = function () {
    ...
    function errorLoad(msg, suggestTryAgain = true) {
        ...
        self.emit("loadFail", "Failed to load channel data from the database: " + msg + extra);
        self.setFlag(Flags.C_ERROR);
    }
    ...
    ChannelStore.load(this.id, this.uniqueName).then(data => {
        Object.keys(this.modules).forEach(m => {
            try {
                ...
                LOGGER.error("Failed to load module " + m + " for channel " + this.uniqueName);
            }
        });

        this.setFlag(Flags.C_READY);
    }).catch(_errors.ChannelStateSizeError, err => {
        const message = "This channel's state size has exceeded the memory limit " + "enforced by this server.  Please contact an administrator " + "for assistance.";
        ...
        if (err.code === 'ENOENT') {
            ...
            const message = "An error occurred when loading this channel's data from " + "disk.  Please contact an administrator for assistance.  " + `The error was: ${err}.`;
...
Channel.prototype.saveState = async function () {
    if (!this.is(Flags.C_REGISTERED)) {
        ...
        throw new Error(`Attempted to save channel ${this.name} ` + `but it wasn't finished loading yet!`);
    }
    ...
    const data = {};
    Object.keys(this.modules).forEach(m => {
        if (this.modules[m].dirty || !this.modules[m].supportsDirtyCheck) {
            this.modules[m].save(data);
        } else {
            LOGGER.debug("Skipping save for %s[%s]: not dirty", this.uniqueName, m);
        }
    });
    ...
    try {
        ...
        if (error instanceof _errors.ChannelStateSizeError) {
...
Channel.prototype.checkModules = function (fn, args, cb) {
    ...
    this.waitFlag(Flags.C_READY, function () {
        ...
        var next = function (err, result) {
            ...
            if (!self.modules) {
                LOGGER.warn('checkModules(%s): self.modules is undefined; dead=%s,' + ' current=%s, remaining=%s', fn, self.dead, m, keys);
                return;
            }
...
console.log("Ju2")
        ...
        user.waitFlag(Flags.U_LOGGED_IN, () => {
            if (self.dead) {
                LOGGER.warn('Got U_LOGGED_IN for %s after channel already unloaded', user.getName());
                return;
            }
console.log("Ju3")
...
Channel.prototype.acceptUser = function (user) {
    ...
    if (user.socket.context.torConnection) {
        if (this.modules.options && this.modules.options.get("torbanned")) {
            ...
            this.logger.log("[login] Blocked connection from Tor exit at " + user.displayip);
            return;
        }

        this.logger.log("[login] Accepted connection from Tor exit at " + user.displayip);
    } else {
    ...
    user.waitFlag(Flags.U_LOGGED_IN, function () {
        if (!self.modules.options.get("allow_anon_chat")) {
            for (var i = 0; i < self.users.length; i++) {
                if (self.users[i] !== user && self.users[i].getLowerName() === user.getLowerName()) {
                    self.users[i].kick("Duplicate login");
                }
            }
        } else {
            for (var i = 0; i < self.users.length; i++) {
                if (self.users[i] !== user && self.users[i].realip == user.realip) {
                    self.users[i].kick("Duplicate login");
                }
            }
        }
...
Channel.prototype.partUser = function (user) {
    ...
    this.logger.log("[login] " + user.displayip + " (" + user.getName() + ") " + "disconnected.");
    user.channel = null;
...
Channel.prototype.maybeResendUserlist = function maybeResendUserlist(user, newRank, oldRank) {
    if (newRank >= 2 && oldRank < 2 || newRank < 2 && oldRank >= 2 || newRank >= 255 && oldRank < 255 || newRank < 255 && oldRank >= 255) {
        this.sendUserlist([user]);
    }
};
...
Channel.prototype.sendUserJoin = function (users, user) {
    ...
    self.modules.chat.sendModMessage(user.getName() + " joined (aliases: " + user.account.aliases.join(",") + ")", 2);
};
